            ...
            if (this.props.selectedNode && this.props.docType === 1) {
                ...
                if (this.props.selectedNode.split('.').length > 3 || !Store.large) {
                    ...
                    segments = EdiHelper.getSegments(json, false).map((v, i) => {
                        return <Segment tabIndex={i} key={v.path} segment={v} type={this.props.docType} selectedSegment={v.path === this.props.selectedSegment} validate={this.props.validate} openModal={this.props.openModal} onSegmentClick={this.props.onSegmentClick} scrollIntoView={this.props.scrollIntoView}/>
                    })
                } else {
                    json = JSPath.apply(this.props.selectedNode, Store.message);
